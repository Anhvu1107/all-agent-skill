...
Use this file before doing substantive work with `security-aws-secrets-rotation`.

## Trigger Summary

ALWAYS use this when the user or an upstream archive refers to `security-aws-secrets-rotation`, the nested skill path form for `aws-secrets-rotation`, or the equivalent slash path. This is a namespace-preserving alias that routes to `aws-secrets-rotation` while retaining strict validation, Senior Master execution, and Codex review standards.
