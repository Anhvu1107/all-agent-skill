...
Use this file before doing substantive work with `game-development-mobile-games`.

## Trigger Summary

ALWAYS use this when the user or an upstream archive refers to `game-development-mobile-games`, the nested skill path form for `mobile-games`, or the equivalent slash path. This is a namespace-preserving alias that routes to `mobile-games` while retaining strict validation, Senior Master execution, and Codex review standards.
