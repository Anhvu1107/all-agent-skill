...
Use this file before doing substantive work with `security-aws-iam-best-practices`.

## Trigger Summary

ALWAYS use this when the user or an upstream archive refers to `security-aws-iam-best-practices`, the nested skill path form for `aws-iam-best-practices`, or the equivalent slash path. This is a namespace-preserving alias that routes to `aws-iam-best-practices` while retaining strict validation, Senior Master execution, and Codex review standards.
