...
Use this file before doing substantive work with `libreoffice-calc`.

## Trigger Summary

ALWAYS use this when the user or an upstream archive refers to `libreoffice-calc`, the nested skill path form for `calc`, or the equivalent slash path. This is a namespace-preserving alias that routes to `calc` while retaining strict validation, Senior Master execution, and Codex review standards.
