...
Use this file before doing substantive work with `security-aws-security-audit`.

## Trigger Summary

ALWAYS use this when the user or an upstream archive refers to `security-aws-security-audit`, the nested skill path form for `aws-security-audit`, or the equivalent slash path. This is a namespace-preserving alias that routes to `aws-security-audit` while retaining strict validation, Senior Master execution, and Codex review standards.
