...
Use this file before doing substantive work with `game-development-game-design`.

## Trigger Summary

ALWAYS use this when the user or an upstream archive refers to `game-development-game-design`, the nested skill path form for `game-design`, or the equivalent slash path. This is a namespace-preserving alias that routes to `game-design` while retaining strict validation, Senior Master execution, and Codex review standards.
