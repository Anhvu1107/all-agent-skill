...
Use this file before doing substantive work with `libreoffice-draw`.

## Trigger Summary

ALWAYS use this when the user or an upstream archive refers to `libreoffice-draw`, the nested skill path form for `draw`, or the equivalent slash path. This is a namespace-preserving alias that routes to `draw` while retaining strict validation, Senior Master execution, and Codex review standards.
